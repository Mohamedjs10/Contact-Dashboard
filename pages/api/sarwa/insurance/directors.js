...
import { si_directors } from "../../../database/schema"; // Model
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const document = new si_directors(body.document);
    ...
    const updatedResult = await si_directors.findByIdAndUpdate(
      { _id: body.id },
      {
        $set: {
          "name.en": name.en,
    ...
    const deletedResult = await si_directors.findByIdAndDelete({
      _id: body.id,
    });
    res.send();
  }
